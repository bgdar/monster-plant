...
func _physics_process(delta: float) -> void:
	if is_move and moster_ui:
		var direction_attack = (moster_ui.global_position - global_position).normalized()
		velocity = direction_attack * SPEED
		move_and_slide()
		human_warior_ui.play("runDown")
		print("human position", global_position)
		print("monster position", moster_ui.global_position)
	else:
